...
| `cv/main_example.md` | Your master markdown CV with actual details |
...
## 7. Export your documents (optional)

`/apply` writes the CV and cover letter as Markdown files (`cv/main_<company>.md` and `cover_letters/cover_<company>_<role>.md`). Most application portals accept PDF; convert with any markdown tool, for example [pandoc](https://pandoc.org/):

```bash
pandoc cv/main_<company>.md -o cv/main_<company>.pdf
pandoc cover_letters/cover_<company>_<role>.md -o cover_letters/cover_<company>_<role>.pdf
```
