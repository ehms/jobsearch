...
files ready    with fit ratings     (Markdown, tailored)
                   |                     |
                   v                     v
...
├── cv/
│   └── main_example.md                # Master markdown CV template
├── cover_letters/                     # Generated markdown cover letters
├── documents/                         # Career source materials for /setup Path A and /expand
│   ├── README.md                      # Folder layout instructions
│   ├── cv/                            # Master CV (PDF or .md)
...
The `/apply` command runs a **drafter-reviewer workflow** with a mandatory final review:
...
3. **Draft** a tailored CV and cover letter in clean Markdown
4. **Spawn a reviewer agent** that researches the company and critiques the drafts
5. **Revise** based on the reviewer's feedback
6. **Review both documents**: Claude re-reads the final files and iterates until the CV fits the ~2-page content budget, the cover letter body is within its 250-300 word budget, and the markdown is clean with no leftover placeholders
...
- **Final document review loop.** Drafts that "looked fine while writing" commonly end up over budget or with broken formatting after revision. The `/apply` command re-reads every final document and iterates until length budgets, structure, and formatting are all clean. This runs automatically on every application.
- **Relevance-weighted CV cutting.** When a CV exceeds the 2-page content budget, the workflow does not cut mechanically from the "oldest" section. It scores each candidate line by (a) relevance to the target posting, (b) uniqueness in the document, and (c) whether the cover letter depends on it, and cuts the lowest-total-score line first. An older-role bullet that hits posting keywords survives ahead of a recent-role bullet that does not.
...
CVs and cover letters are generated as clean Markdown, structured by the guidance in `05-cv-templates.md` and `06-cover-letter-templates.md`. You can adjust the structure, section order, or formatting conventions by editing those two files. The markdown converts cleanly to PDF or DOCX with any standard tool (e.g. [pandoc](https://pandoc.org/)) when an employer needs a formatted document.
