...
- `cover_letter.md` (or legacy `cover_letter.tex`): opening structure, body structure, bullet style, closing, recurring phrases
- `cv_draft.md` (or legacy `cv_draft.tex`): profile statement, section ordering, framing for this role type
...
- **`03-writing-style.md`:** Source is past cover letter files (`cover_letter.md` / legacy `.tex`). Extract recurring patterns. Add as observations under "## Patterns Observed in Past Applications". Do not modify existing rules. Only add if 2+ cover letters show a genuine pattern.
...
- **`05-cv-templates.md`:** Source is past CV draft files (`cv_draft.md` / legacy `.tex`). Extract any profile statement that does not already appear in templates. Label with: *[Used for: <company>_<role>]*
- **`06-cover-letter-templates.md`:** Source is past cover letter files (`cover_letter.md` / legacy `.tex`). Extract opening patterns, bullet structures, closing formulations. Add only what is structurally distinct from existing templates.
...
Then proceed to Step 3 to populate the non-skill files (`CLAUDE.md`, `cv/main_example.md`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
> - `cv/main_example.md` - Your master CV template
